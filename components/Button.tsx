...
export default function Button({ href, children, variant = "primary", size = "md" }: Props) {
    const cls =
        variant === "primary"
            ? `${base} ${sizes[size]} text-white`
            + " bg-[var(--brand-blue)] hover:bg-[#0858d8] focus-visible:ring-[var(--brand-blue)]"
            : variant === "accent"
                ? `${base} ${sizes[size]} text-black`
                + " bg-[var(--brand-yellow)] hover:bg-[#e6af12] focus-visible:ring-[var(--brand-yellow)]"
